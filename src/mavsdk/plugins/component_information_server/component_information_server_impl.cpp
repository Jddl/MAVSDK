...
ComponentInformationServerImpl::provide_float_param(ComponentInformationServer::FloatParam param)
{
    ...
    _parent->subscribe_param_float(param.name, [this, name = param.name](float new_value) {
        param_update(name, new_value);
    }, this);

    return ComponentInformationServer::Result::Success;
}

void ComponentInformationServerImpl::subscribe_float_param(ComponentInformationServer::FloatParamCallback callback)
{
    std::lock_guard<std::mutex> lock(_mutex);
